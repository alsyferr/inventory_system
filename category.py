...
def select_data(
    event, invoice_entry, name_entry, description_text, treeview
):
    index = treeview.selection()

    if not index:  # Ensure an item is selected
        return
    content = treeview.item(index[0])  # Extract the first selected item
...
def clear(id_entry, name_entry, description_text, treeview):
...
def category_form(window):
    ...
    category_name_entry.grid(row=1, column=1,pady=10)
    ...
    clear_button = Button(
        ...
        fg="white",
        bg="#0f4d7d",
        command=lambda :clear(id_entry,category_name_entry,description_text,treeview)
    ...
    treeview.bind(
        ...
        lambda event: select_data(
            event, id_entry, category_name_entry, description_text, treeview
        ),
    )
